...
rs.open "select * from log where kind = 'product' order by dates desc",conn
	if not rs.eof then
		rs1.open "select * from log where kind = 'product' and dates = '"+rs("dates")+"' order by counts desc",conn,1,3
		if not rs1.eof then
			%>
			<div style="width:100%">
				<table border="0" cellspacing="0" width="701" bordercolor="#000000" cellpadding="0" align="left" style="margin:0 0 0 10px; font-size:14px;">
					...
					<tr> 
						...
						<td bgcolor="#FFFCD9" style="border:1px #000000 solid"><p align="center"><a href="log_product_view.asp?action=search&name=<%=rs1("name")%>">View</a> 
						</td>
					</tr>
					<%
					rs1.movenext
					...
					<tfoot>
						...
						if curpage = 1 then
							...
							response.write "<a href='log_product_view.asp?&page=1'>First</a> <a href='log_view.asp?page=" + cstr(curpage-1) + "'>Previous</a> "                    
						end if                                                     
		
						if  curpage = rs1.pagecount then                            
							response.write "&nbsp;&nbsp;"                           
						else                                                  
							response.write "<a href='log_product_view.asp?page=" + cstr(curpage+1) + "'>Next</a> <a href='log_product_view.asp?page=" + cstr(rs1.pagecount)  + "'>Last</a></span>"
						end if                                                      
			end if
			rs1.close
						%></td>
