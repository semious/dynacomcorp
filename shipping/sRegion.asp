...
' sRegion.asp - Vendor By Region Lookup

Class clsShippingRateByRegion

	' Constructor -----------------------------------------------------	

	Private Sub Class_Initialize()
		Name          = "By Region Shipping"
		Installed     = True
		Debug         = False
	...
	Public Sub AddRates(ByVal argShipToInfo, ByVal argCart, ByRef argShipChoiceTable, ByRef argErrorMsg)
		Dim SQL, Key, rsMethods, rsRegion

		On Error Resume Next

		SQL = "Select * From ShippingRegions " &_
				"WHERE {fn LCase(StateCode)} = '" & LCase(argShipToInfo.DestState) & "'"

		set rsRegion = Server.CreateObject("ADODB.Recordset")
		rsRegion.Open SQL, Conn, adOpenKeyset, adLockOptimistic

		If (rsRegion.RecordCount <> 0) Then
			SQL = "SELECT * FROM ShippingMethods " & _
			 		"WHERE {fn LCase(Gateway)} = 'byregion' AND {fn LCase(ShippingMethodActive)} = 'yes'"
			...
			Do While (Not rsMethods.EOF)
				...
				argShipChoiceTable.Item(Key).Rate        = CDbl(rsRegion("CostPerUnitMethod" & CInt(rsMethods("ShipperCode")))) * _
				                                           CDbl(argShipToInfo.OrderShipUnits)
...
Set FeatureInstalled.Item("Rate Gateway:ByRegion") = New clsShippingRateByRegion
